...
#Initialize the leg calculator for leg-based calculations
leg_calculator =  LegCalculator()
...
try:
    ...
    dep_results = pd.read_csv("C:/Users/vladc/OneDrive/Escritorio/CodingProjects/SustainableFestivals/FirstOrderSimulation/Data/GeneratedInitialTrips/departure_trips.csv")
    ret_results = pd.read_csv("C:/Users/vladc/OneDrive/Escritorio/CodingProjects/SustainableFestivals/FirstOrderSimulation/Data/GeneratedInitialTrips/return_trips.csv")
    ...
    least_leg_dep = leg_calculator.select_least_leg_options(dep_costs)
    least_leg_ret = leg_calculator.select_least_leg_options(ret_costs)

     # Download the lowest cost options to CSV files
    least_leg_dep.to_csv("Data/ChosenTrips/least_legs_departure.csv", index=False)
    least_leg_ret.to_csv("Data/ChosenTrips/least_legs_return.csv", index=False)

    # Perform transportation analysis using the lowest cost options
    analysis = StatisticalAnalysis(least_leg_dep, least_leg_ret)

    # Analyze the data with all options: compute metrics, save to CSV, and print summary
    metrics = analysis.analyze(
        output_csv="Data/FinalStatistics/CSVs/min_legs_metrics.csv",  # Optional: also save as CSV
        output_md="Data/FinalStatistics/MDs/min_legs_report.md",     # Save as Markdown
