...
class TripDataProcessor:
    ...
    def extract_leg_data(self, leg: Dict[str, Any], leg_number: int) -> Dict[str, Any]:
        ...
        step_names = [
            step.get("streetName", "").strip()
            for step in leg.get("steps", [])
            if step.get("streetName")
        ]
        road_list = ", ".join(step_names) if step_names else "Unknown"

        route = leg.get("route") or {}
        route_short = route.get("shortName", "").strip()
        route_long = route.get("longName", "").strip()
        if route_short and route_long:
            if route_short in route_long:
                route_name = route_long
            else:
                route_name = f"{route_long} {route_short}"
        elif route_short:
            route_name = route_short
        elif route_long:
            route_name = route_long
        else:
            route_name = ""
        ...
        return {
            ...
            f"leg{leg_number}_roads": road_list,
            f"leg{leg_number}_route": route_name  # <--- new line
        }
